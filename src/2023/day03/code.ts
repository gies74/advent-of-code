...
namespace day03 {
    ...
    Utils.main(
        ...
        (input: string[], part: Part) => {
            ...
            /* part 2 */
            const counts = Array(input.length).fill(0).map(l => Array(input[0].length).fill(null).map(c => []));
                ...
                matches.forEach(m => {
                    ...
                    for (var idx=m.index; idx<m.index + m[0].length; idx++) {
                        deltas.forEach(d => {
                            ...
                            touchSym ||= /[^.^\d]/.test(input[li+d[0]][idx+d[1]]);
                    ...
                    starCoords.forEach(c => {
                        counts[c[0]][c[1]].push(parseInt(m[0]));
                    });
                });
            });

            let cum = 0;
            counts.forEach((r,ri) => r.forEach((c,ci) => {
                const coord = [ri, ci];
                if (c.length == 2) {
                    cum += c[0] * c[1];
                }
